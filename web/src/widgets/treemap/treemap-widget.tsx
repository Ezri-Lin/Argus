...
function fitTextSize(text: string, desired: number, maxW: number, min: number, max: number): number {
      ...
      >
            ...
            return (
              ...
              >
                <rect
                  ...
                  height={cell.h}
                  rx={radius.inner}
                  fill={hovered?.item.name === cell.item.name ? s.fill.replace(/[\d.]+\)$/, (m) => `${Math.min(parseFloat(m) * 2.5, 0.45)})`) : s.fill}
                  stroke={hovered?.item.name === cell.item.name ? s.text : s.border}
                  strokeWidth={hovered?.item.name === cell.item.name ? 2.5 : 1.5}
                  strokeDasharray={
        ...
        {hovered && (
          <PopupBubble
            ...
            y={hovered.y > 60 ? hovered.y : hovered.y + hovered.h}
            anchor={hovered.y > 60 ? "top" : "bottom"}
          >
            <div style={{ fontWeight: 600, fontSize: 13, marginBottom: 4 }}>{hovered.item.name}</div>
            <div style={{ color: color.textSecondary, fontSize: 11 }}>
              {hovered.item.metric} · value {hovered.item.value} · heat{" "}
              {hovered.item.heat.toFixed(2)}
            </div>
          </PopupBubble>
        )}
      </div>
