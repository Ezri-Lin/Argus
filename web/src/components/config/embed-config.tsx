...
export function EmbedConfig({
  ...
  return (
    <>
      {/* Source Mode Selector — always visible, drives everything below */}
      {followRule && setFollowRule && (
        <SourceModeSelector followRule={followRule} setFollowRule={setFollowRule} />
      )}

      {/* Manual mode: URL parse + sources editor */}
      {isManual && (
        <>
                ...
                <div key={i} style={{ padding: 8, background: color.surface2, borderRadius: radius.inner, border: `1px solid ${color.hairline}` }}>
                  <div className="flex items-center gap-1 mb-1.5">
                    ...
                    {s.health && s.health !== "ok" && (
                      <span style={{
                        padding: "2px 6px", borderRadius: 999, fontSize: 10, flexShrink: 0,
                        background: s.health === "stale" ? "rgba(245,158,11,0.15)" : "rgba(239,68,68,0.15)",
                        color: s.health === "stale" ? "#f59e0b" : "#ef4444",
                        border: `1px solid ${s.health === "stale" ? "rgba(245,158,11,0.3)" : "rgba(239,68,68,0.3)"}`,
                      }}>
                        {s.health === "stale" ? "re-parsed" : "dead"}
                      </span>
                    )}
      ...
      {/* Follow mode: read-only sources preview */}
      {!isManual && sources.length > 0 && (
        <div>
          <label style={{ fontSize: 11, color: color.textMuted, marginBottom: 6, display: "block" }}>Current sources</label>
          <div className="flex flex-col gap-1">
            {sources.map((s, i) => (
              <div key={i} style={{
                display: "flex", alignItems: "center", gap: 6, padding: "5px 8px",
                background: color.surface2, borderRadius: radius.inner, border: `1px solid ${color.hairline}`,
              }}>
                <span style={{
                  width: 6, height: 6, borderRadius: 999, flexShrink: 0,
                  background: !s.health || s.health === "ok" ? color.pos : s.health === "stale" ? "#f59e0b" : "#ef4444",
                }} />
                <span style={{ fontSize: 12, color: color.textPrimary, flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }} title={s.fullLabel ?? s.label}>
                  {s.label}
                </span>
                {s.type && (
                  <span style={{ padding: "1px 5px", borderRadius: 999, background: color.surfaceElev, border: `1px solid ${color.hairline}`, color: color.textMuted, fontSize: 9, textTransform: "uppercase" }}>
                    {s.type}
                  </span>
                )}
              </div>
